...
inline uint32_t offset(uint32_t x, uint32_t y, uint32_t m) {
	return x * m + y;
}
...
#pragma omp parallel for collapse(2) shared(val, val_new) reduction(max: error)
	for (size_t j = 1; j < n - 1; ++j) {
		for (size_t i = 1; i < m - 1; ++i) {
			...
			val_new[offset(j, i, m)] = 0.25 * (val[offset(j, i + 1, m)] + val[offset(j, i - 1, m)] + val[offset(j - 1, i, m)] + val[offset(j + 1, i, m)]);
